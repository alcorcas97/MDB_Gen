...
(setq fmdb-decisions nil fmdb-highlighted nil fmdb-preview-objects nil)
...
(defun fmdb-midpoint (a b)
  (list (/ (+ (car a) (car b)) 2.0) (/ (+ (cadr a) (cadr b)) 2.0) (/ (+ (if (caddr a) (caddr a) 0.0) (if (caddr b) (caddr b) 0.0)) 2.0)))
(defun fmdb-pan-to-pair (a b / app center viewSize result)
  (setq app (vlax-get-acad-object) center (fmdb-midpoint a b) viewSize (getvar "VIEWSIZE"))
  (setq result (vl-catch-all-apply 'vla-ZoomCenter (list app (vlax-3d-point center) viewSize)))
  (if (vl-catch-all-error-p result) nil T))
(defun fmdb-preview-layer (/ document layers layer result)
  (setq document (vla-get-ActiveDocument (vlax-get-acad-object)) layers (vla-get-Layers document))
  (setq result (vl-catch-all-apply 'vla-Item (list layers "FMDB_PHKT_PREVIEW")))
  (setq layer (if (vl-catch-all-error-p result) (vla-Add layers "FMDB_PHKT_PREVIEW") result))
  (vl-catch-all-apply 'vla-put-PlotStyleName (list layer "Normal"))
  (vl-catch-all-apply 'vla-put-Plottable (list layer :vlax-false))
  layer)
(defun fmdb-clear-preview ()
  (foreach object fmdb-preview-objects (if object (vl-catch-all-apply 'vla-Delete (list object))))
  (setq fmdb-preview-objects nil))
(defun fmdb-add-preview (source target / document space layer line circle)
  (fmdb-clear-preview)
  (setq document (vla-get-ActiveDocument (vlax-get-acad-object))
        space (vla-get-ModelSpace document)
        layer (vla-get-Name (fmdb-preview-layer)))
  (setq line (vla-AddLine space (vlax-3d-point source) (vlax-3d-point target)))
  (vla-put-Layer line layer)
  (vla-put-Color line 4)
  (vla-put-Lineweight line 50)
  (setq circle (vla-AddCircle space (vlax-3d-point target) (max 0.6 (* 0.018 (fmdb-distance source target)))))
  (vla-put-Layer circle layer)
  (vla-put-Color circle 1)
  (vla-put-Lineweight circle 50)
  (setq fmdb-preview-objects (list line circle))
  (redraw))
...
(defun fmdb-assignment-count (candidateId / count decision)
  (setq count 0)
  (foreach decision fmdb-decisions (if (and (= (cadr decision) candidateId) (= (nth 5 decision) "accepted")) (setq count (1+ count)))) count)
(defun fmdb-preview (text candidate / object textPoint target)
  (fmdb-unhighlight)
  (fmdb-highlight (car text))
  (foreach handle (nth 5 candidate) (fmdb-highlight handle))
  (setq object (fmdb-object (car text)) textPoint (if object (fmdb-logical-point object) (nth 2 text)) target (nth 1 candidate))
  (if (and textPoint target) (progn (fmdb-pan-to-pair textPoint target) (fmdb-add-preview textPoint target))))
...
(defun fmdb-set-decision (text candidate source target method status)
  (fmdb-remove-decision (car text))
  (setq fmdb-decisions (cons (list (car text) (if candidate (car candidate) "") source target method status) fmdb-decisions)))
(defun fmdb-move-object (object source target / result)
  (setq result (vl-catch-all-apply 'vla-Move (list object (vlax-3d-point source) (vlax-3d-point target))))
  (if (vl-catch-all-error-p result) (vl-catch-all-error-message result) nil))
(defun fmdb-move-text-now (text candidate method / object source target errorText previous)
  (setq previous (fmdb-find-decision (car text)))
  (if previous (fmdb-restore-decision previous))
  (setq object (fmdb-object (car text)) source (if object (fmdb-logical-point object) nil) target (if candidate (nth 1 candidate) nil))
  (if (and object source target)
    (progn
      (fmdb-clear-preview)
      (setq errorText (fmdb-move-object object source target))
      (if errorText
        (progn (prompt (strcat "\nNo se pudo mover el texto: " errorText)) nil)
        (progn (fmdb-set-decision text candidate source target method "accepted") T)))
    (progn (prompt "\nNo se pudo resolver el texto seleccionado.") nil)))
(defun fmdb-restore-decision (decision / object source target)
  (if (and decision (= (nth 5 decision) "accepted"))
    (progn
      (setq object (fmdb-object (car decision)) source (nth 2 decision) target (nth 3 decision))
      (if (and object source target) (fmdb-move-object object target source)))))
(defun fmdb-rollback-decisions ()
  (foreach decision fmdb-decisions (fmdb-restore-decision decision))
  (setq fmdb-decisions nil))
...
(defun fmdb-summary-values (/ assigned manual skipped noCandidates shared maximum candidateId count counts)
  ...
  (foreach decision fmdb-decisions
    (cond ((= (nth 5 decision) "accepted")
      (setq assigned (1+ assigned)) (if (= (nth 4 decision) "manual") (setq manual (1+ manual)))
      ...
      ((= (nth 5 decision) "skipped") (setq skipped (1+ skipped)))))
...
(defun fmdb-save-document (/ document result)
  (setq document (vla-get-ActiveDocument (vlax-get-acad-object)))
  (setq result (vl-catch-all-apply 'vla-Save (list document)))
  (if (vl-catch-all-error-p result) (vl-catch-all-error-message result) nil))

(defun c:${commandName} (/ total index text candidates candidateIndex candidate choice done cancelled manualCandidate summary saveError document oldError *error*)
  (setq oldError *error*)
  (setq *error* (lambda (message)
    (vl-catch-all-apply 'fmdb-unhighlight nil)
    (vl-catch-all-apply 'fmdb-rollback-decisions nil)
  ...
  (while (and (< index total) (not done) (not cancelled))
    ...
    (if candidates
      (while (and (not choice) (not done) (not cancelled))
        ...
        (cond
          ((= choice "Aceptar") (if (fmdb-move-text-now text candidate "automatic") (setq index (1+ index) choice "accepted") (setq choice nil)))
          ...
          ((= choice "Manual") (setq manualCandidate (fmdb-manual text))
            (if manualCandidate
              (if (fmdb-move-text-now text manualCandidate "manual") (setq index (1+ index) choice "accepted") (setq choice nil))
              (setq choice nil)))
          ((= choice "Omitir") (fmdb-set-decision text nil nil nil "none" "skipped") (setq index (1+ index)))
          ((= choice "Volver") (if (> index 0) (progn (setq index (1- index)) (fmdb-restore-decision (fmdb-find-decision (car (nth index fmdb-texts)))) (fmdb-remove-decision (car (nth index fmdb-texts))))) )
      ...
      (progn
        ...
        (cond ((= choice "Manual") (setq manualCandidate (fmdb-manual text))
                (if manualCandidate (if (fmdb-move-text-now text manualCandidate "manual") (setq index (1+ index)))))
              ((= choice "Omitir") (fmdb-set-decision text nil nil nil "none" "skipped") (setq index (1+ index)))
              ((= choice "Volver") (if (> index 0) (progn (setq index (1- index)) (fmdb-restore-decision (fmdb-find-decision (car (nth index fmdb-texts)))) (fmdb-remove-decision (car (nth index fmdb-texts))))))
  ...
  (if cancelled
    (progn
      (fmdb-rollback-decisions)
      (setq summary (fmdb-summary-values))
      (fmdb-write-result "CANCELLED" summary nil)
      (prompt "\nOperacion cancelada. Se han revertido los movimientos de esta tanda."))
    (progn
      ...
      (fmdb-stage "save")
      (setq saveError (fmdb-save-document))
      (if saveError
        (progn
          (fmdb-rollback-decisions)
          (setq summary (list (nth 0 summary) (nth 1 summary) (nth 2 summary) (nth 3 summary) (nth 4 summary) 1 (nth 6 summary) (nth 7 summary)))
          (fmdb-write-result "ROLLED_BACK" summary saveError))
        (fmdb-write-result "APPLIED" summary nil))))
